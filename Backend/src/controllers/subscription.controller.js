...
class Subscription {
    static async postToken(req, res) {
        new SuccessResponse({
            message: _SubscriptionService.postToken(req.user, req.body.token, req.body.plan),
        }).send(res)
    }
    ...
    static async studentSub(req, res) {
        new SuccessResponse({
            ...
            data: await SubscriptionService.studentSub({user: req.user, studentInfo: req.body})
        }).send(res)
    }

    static async verifyStudent(req, res) {
        new SuccessResponse({
            message: await SubscriptionService.studentVerify({user: req.user, OTP: req.body.OTP, token: req.headers.token})
        }).send(res)
    }
}
